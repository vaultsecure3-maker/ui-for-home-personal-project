...
import { Search, X, Plus, Pencil, Trash2, FileText, FileType, ChevronLeft, ChevronRight } from 'lucide-react';
import { mockDocumentTypes, DocumentType } from '../../lib/mockData';
...
export function DocumentTypesView() {
  ...
  const sortedTypes = [...mockDocumentTypes]
    .filter((type) =>
  ...
  const handleSaveType = () => {
    if (editingType) {
      toast.success(`Document type "${typeName}" updated`);
    } else {
      toast.success(`Document type "${typeName}" created`);
    }
  ...
  const handleViewDocuments = (type: DocumentType) => {
    navigate(`/documents?type=${type.id}`);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="bg-white border-b border-gray-200 px-8 py-6 flex-shrink-0">
        ...
        <p className="text-sm text-gray-600 mt-1">Categorize documents by their type and purpose</p>
      </div>
        ...
        <div className="flex items-center gap-6 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-600">
              <span className="font-medium text-gray-900">{sortedTypes.length}</span> document types
            </span>
          </div>
          {searchQuery && (
      ...
      <div className="flex-1 overflow-auto p-6">
                ...
                >
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      <span>{type.documentCount} documents</span>
                    </div>
                  </TableCell>
        ...
        {sortedTypes.length === 0 && (
          <div className="text-center py-12">
            <FileType className="h-12 w-12 text-gray-300 mx-auto mb-4" />
      ...
      {sortedTypes.length > 0 && (
        <div className="bg-white border-t border-gray-200 px-6 py-3 flex items-center justify-between">
          <div className="text-sm text-gray-600">
            ...
            {Math.min(currentPage * pageSize, sortedTypes.length)} of{' '}
            {sortedTypes.length} document types
          </div>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {editingType
                ...
                : 'Add a new document type to categorize your documents'}
            </DialogDescription>
          </DialogHeader>
                ...
                {presetColors.map((color) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full transition-transform ${
                      typeColor === color ? 'scale-110 ring-2 ring-offset-2 ring-gray-400' : ''
                    }`}
