...
import { Search, X, LayoutGrid, List, ChevronLeft, ChevronRight, Filter, Check, ChevronsUpDown } from 'lucide-react';
...
export function DocumentsView() {
  ...
  const [correspondentsOpen, setCorrespondentsOpen] = useState(false);
  ...
  const toggleCorrespondentFilter = (corrId: number) => {
  ...
  const toggleDocumentTypeFilter = (typeId: number) => {
        ...
        <div className="bg-gray-50 border-b border-gray-200 p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-medium text-gray-700">Contains Word</label>
            ...
            {/* Correspondents Combobox */}
            <div className="flex flex-col gap-2">
              <label className="text-xs font-medium text-gray-700">Correspondents</label>
              <Popover open={correspondentsOpen} onOpenChange={setCorrespondentsOpen}>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    role="combobox"
                    aria-expanded={correspondentsOpen}
                    className="justify-between"
                  >
                    {selectedCorrespondents.length > 0
                      ? `${selectedCorrespondents.length} selected`
                ...
                <PopoverContent className="w-[300px] p-0" align="start">
                  <Command>
                    <CommandInput placeholder="Search correspondents..." />
                    <CommandList>
                      <CommandEmpty>No correspondents found.</CommandEmpty>
                      <CommandGroup>
                        {mockCorrespondents.map((correspondent) => (
                          <CommandItem
                            key={correspondent.id}
                            value={correspondent.name}
                            onSelect={() => toggleCorrespondentFilter(correspondent.id)}
                          >
                            <Check
                              className={cn(
                                "mr-2 h-4 w-4",
                                selectedCorrespondents.includes(correspondent.id) ? "opacity-100" : "opacity-0"
                              )}
                            />
                            <div
                              className="w-3 h-3 rounded-full mr-2"
            ...
            {/* Document Types Combobox */}
            <div className="flex flex-col gap-2">
              <label className="text-xs font-medium text-gray-700">Document Types</label>
                      ...
                      <CommandGroup>
                        {mockDocumentTypes.map((type) => (
                          <CommandItem
                            key={type.id}
                            value={type.name}
                            onSelect={() => toggleDocumentTypeFilter(type.id)}
                          >
                            <Check
                              className={cn(
                                "mr-2 h-4 w-4",
                                selectedDocumentTypes.includes(type.id) ? "opacity-100" : "opacity-0"
                              )}
                            />
                            <div
                              className="w-3 h-3 rounded-full mr-2"
          ...
          {selectedCorrespondents.map((corrId) => {
            ...
            return corr ? (
              <Badge key={`corr-${corrId}`} variant="secondary" className="gap-1">
                ...
                <button
                  onClick={() => toggleCorrespondentFilter(corrId)}
                  className="ml-1 hover:bg-gray-300 rounded-full p-0.5"
          ...
          {selectedDocumentTypes.map((typeId) => {
            ...
            return type ? (
              <Badge key={`type-${typeId}`} variant="secondary" className="gap-1">
                Type: {type.name}
                <button
                  onClick={() => toggleDocumentTypeFilter(typeId)}
                  className="ml-1 hover:bg-gray-300 rounded-full p-0.5"
